...
template<typename E,
         E Min = (E)std::max(
           (ssize_t)std::numeric_limits<std::underlying_type_t<E>>::min(),
           (ssize_t)-300),
         E Max = (E)std::min(
           (ssize_t)std::numeric_limits<std::underlying_type_t<E>>::max(),
           (ssize_t)300)>
struct enum_traits
{
  ...
  private:
    ...
    explicit const_iterator(UnderlyingIter val)
      : _M_val(val)
    {
    }
  ...
  template<typename T>
  static bool has_value(T e)
  {
    return name.find((E)e) != name.end();
  }
  template<typename F>
...
auto get_enum_names()
{
  ...
  constexpr ssize_t min =
    std::max((ssize_t)std::numeric_limits<T>::min(), (ssize_t)Min);
  constexpr ssize_t max =
    std::min((ssize_t)std::numeric_limits<T>::max(), (ssize_t)Max);
  return get_enum_names_sequence<E, ssize_t, min>(
    std::make_integer_sequence<ssize_t, max - min>{});
}
...
auto get_all_values()
{
  ...
  auto& _name = enum_traits<E, Min, Max>::name;
  for (auto s = _name.rbegin(); s != _name.rend(); ++s) {
    values.emplace_back(s->first);
  }
  return values;
}

template<typename E, E Min, E Max>
const std::map<E, std::string_view> enum_traits<E, Min, Max>::name =
  get_enum_names<E, Min, Max>();
template<typename E, E Min, E Max>
const std::unordered_map<std::string_view, E> enum_traits<E, Min, Max>::value =
  get_enum_values<E, Min, Max>();
template<typename E, E Min, E Max>
const std::string_view enum_traits<E, Min, Max>::type_name =
  type_reflection<E>::name;
template<typename E, E Min, E Max>
const std::vector<E> enum_traits<E, Min, Max>::all =
  get_all_values<E, Min, Max>();
