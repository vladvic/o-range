...
public:
  PrintCommand(const std::string& what,
               PrintCommandType type = PrintCommandType::PRINT_OUT_COMMAND)
    : m_what(what)
...
class SimpleSubscriber : public CommandBusSubscriber
{
public:
...
int main()
{
  ...
  auto sub = std::make_shared<SimpleSubscriber>();
  {
    ...
    for (const auto& type : std::ranges::subrange(print_command_traits::begin(),
                                                  print_command_traits::end()))
    {
      std::cout << (int)type << ": " << print_command_traits::get_name(type)
                << std::endl;
    }

    for (int i = 0; i < 10; ++i) {
      bus.publish(std::make_unique<PrintCommand>("Hello world " +
                                                 std::to_string(i) + "!"));
    }
    for (int i = 0; i < 10; ++i) {
      bus.publish(
        std::make_unique<PrintCommand>("Hello world " + std::to_string(i) + "!",
                                       PrintCommandType::PRINT_ERR_COMMAND));
    }
    for (int i = 0; i < 10; ++i) {
      bus.publish(std::make_unique<WriteCommand>("Hello world " +
                                                 std::to_string(i) + "!"));
    }
  }
}
