...
void printBacktrace(int levels)
{
    ...
    for (int i = SHIFT; i < frames; ++i)
    {
        ...
        for (int t = 0; symbol_str[t] && !((start_mangled >= 0) && (end_mangled >= 0)); ++t)
        {
            if (symbol_str[t] == '(')
            {
                start_mangled = t;
        ...
        if (start_mangled >= 0 && end_mangled >= 0 && start_mangled < end_mangled)
        {
            ...
            int status;
            demangled_name = abi::__cxa_demangle(symbol_str, demangled_name, &demangled_length, &status);
            ...
            if (status == 0)
            {
                LOG_INFO("{}: {}({}{}", i - SHIFT, symbols[i], demangled_name, symbol_str).show();
            }
            else
            {
